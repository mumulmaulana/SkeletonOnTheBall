...
"""
Imported from https://github.com/stefanopini/simple-HRNet
Modifications:
- Customization of drawing functions
"""
...
def get_points_and_skeleton_color(skeleton_color_palette='Set2', skeleton_palette_samples=8, person_index=0):
    """
    Get color from specified color palette according to the person index
    ...
    Returns:
        Tuple that contains the color for the specified person
    """
    try:
...
def draw_id(image, points, skeleton_colors, person_index=0):
    cv2.putText(image, 'ID: %d' % person_index, (int(points[1]), int(points[0])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, tuple(skeleton_colors), 2)
    return image

def draw_bbox(image, bbox, skeleton_colors):
    x1, y1, x2, y2 = bbox
    image = cv2.rectangle(image, (x1, y1), (x2, y2), tuple(skeleton_colors), 2)
    return image

def draw_points_and_skeleton_v2(image, points, skeleton, skeleton_colors, point_colors, confidence_threshold=0.5):
    """
    ...
    Args:
        ...
        skeleton_colors: obtained from get_points_and_skeleton_color(). format: (b, g, r)
        point_colors: obtained from get_points_and_skeleton_color(). format: (b, g, r)
    ...
    for joint in skeleton:
        ...
        if pt1[2] > confidence_threshold and pt2[2] > confidence_threshold:
            ...
            image = cv2.circle(image, (int(pt1[1]), int(pt1[0])), 2, (0, 0, 255), -1) # red, change to point_colors if needed
            image = cv2.circle(image, (int(pt2[1]), int(pt2[0])), 2, (0, 0, 255), -1)
    return image
