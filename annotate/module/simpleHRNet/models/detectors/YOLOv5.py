...
class YOLOv5:
    def __init__(self,
                 model_def='',
                 model_folder='./module/simpleHRNet/models/detectors/yolov5',
    ...
    def predict_single_yolov5format(self, image, color_mode='BGR'):
        image = image.copy()
        if self.trt_model:
            # when running with TensorRT, the image must have fixed size
            image, (ratiow, ratioh), (dw, dh) = letterbox(image, self.image_resolution, stride=self.model.stride,
        ...
        if self.trt_model:
            # account for the image resize fixing the xyxy locations
            detections.xyxy[0, :, [0, 2]] = (detections.xyxy[0, :, [0, 2]] - dw) / ratiow
            detections.xyxy[0, :, [1, 3]] = (detections.xyxy[0, :, [1, 3]] - dh) / ratioh
