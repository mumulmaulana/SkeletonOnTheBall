...
class SimpleHRNet:
    ...
    def _predict_single(self, image):
        if not self.multiperson:
            ...
            detections = self.detector.predict_single_yolov5format(image)

            # Supervision ByteTracker
            detections = sv.Detections.from_yolov5(detections)
            detections = self.tracker.update_with_detections(detections)

            nof_people = len(detections.xyxy) if detections is not None else 0
            boxes = np.empty((nof_people, 5), dtype=np.int32) # [x1, y1, x2, y2, track_id]
            ...
            if detections is not None:
                for i, (xyxy, track_id) in enumerate(zip(detections.xyxy, detections.tracker_id)):
                    ...
                    boxes[i] = [x1_new, y1_new, x2_new, y2_new, track_id]
